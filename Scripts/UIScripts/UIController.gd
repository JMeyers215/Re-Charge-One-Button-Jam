...
func _physics_process(_delta: float) -> void:
	...
	if Input.is_action_pressed("one_button") && (main.paused == true || main.game_win == true):
		select_counter += 1
...
func _input(_event: InputEvent) -> void:
	if Input.is_action_just_released("one_button") && (main.paused == true || main.game_win == true):
		select_counter = 0
